...
export function searchKnowledgeBase(query) {
  ...
  keywords.forEach(keyword => {
    ...
    // Stem matching: try common English suffixes
    if (keywordLower.length >= 4) {
      const suffixes = ['s', 'es', 'ed', 'ing', 'er', 'tion', 'ment', 'ness', 'ly'];
      const stems = new Set();
      for (const suf of suffixes) {
        if (keywordLower.endsWith(suf) && keywordLower.length > suf.length + 2) {
          stems.add(keywordLower.slice(0, -suf.length));
        }
        stems.add(keywordLower + suf);
      }
      for (const variant of stems) {
        if (variant !== keywordLower && invertedIndex[variant]) {
          invertedIndex[variant].forEach(match => {
            const key = `${match.fileName}:${match.sectionIdx}`;
            if (!matches.has(key)) {
              matches.set(key, { ...match, score: 0 });
            }
