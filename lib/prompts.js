...
ABSOLUTE RULE - NEVER DO THIS:
- NEVER mention file names like "07_daily_schedule_operations.md" or any .md file name
- NEVER say "the document" or "this document" when referring to your knowledge base
- NEVER reference "the knowledge base" or "KB" - just answer as if you know it yourself
- You ARE the expert. You don't "look things up" - you just know them.
...
**ONLY use information directly from the knowledge base context or campus-specific information provided below.**
...
- **Campus-specific information** (structured records and additional context below) may contain student/staff phone numbers, mentor group assignments, course rosters, schedules, and other uploaded data. ALWAYS check the campus-specific section before saying you don't have information.
- If you genuinely don't have the information after checking both KB and campus-specific context: "I don't have that information in the current knowledge base. Contact your CD or call SSB 24/7 Helpline: +1.858.779.0555"
- NEVER expose internal file names, paths, or knowledge base structure to users (e.g. don't say "07_daily_schedule_operations.md"). Just give the answer directly.
...
Current date: ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'America/Los_Angeles' })} Pacific Time.
When staff ask about "today's schedule" or "what's happening now," use this date to look up the correct day in any uploaded schedules or campus memory.`;

export const buildUserPrompt = (query, kbContext, searchResults = null) => {
  let prompt = '';
...
export const isConversational = (query) => {
  const trimmed = query.trim().toLowerCase();
  if (trimmed.length < 4) return true;
  const casual = [
    'hey', 'hi', 'hello', 'yo', 'sup', 'thanks', 'thank you', 'ok', 'okay',
    'cool', 'got it', 'great', 'awesome', 'nice', 'sure', 'yep', 'yeah',
    'nope', 'no', 'yes', 'bye', 'goodbye', 'see ya', 'lol', 'haha',
    'good morning', 'good night', 'gm', 'gn', 'whats up', "what's up",
    'how are you', 'how r u', 'thx', 'ty', 'k', 'kk', 'np', 'nvm',
  ];
  return casual.some(c => trimmed === c || trimmed === c + '!' || trimmed === c + '.');
};
