...
import { rebuildCampusMemoryIndex, searchCampusMemory } from './campusMemoryIndex.js';
...
let campusMemoryCache = { expiry: 0, indexed: false };

async function ensureCampusMemoryIndex() {
  const now = Date.now();
  if (campusMemoryCache.indexed && now < campusMemoryCache.expiry) return;

  const memories = await getCampusMemory();
  rebuildCampusMemoryIndex(memories);
  campusMemoryCache = { expiry: now + 300000, indexed: true };
}

export function invalidateCampusMemoryIndex() {
  campusMemoryCache = { expiry: 0, indexed: false };
}

export async function getCampusMemoryContext(query = '') {
  await ensureCampusMemoryIndex();
  return searchCampusMemory(query);
}
