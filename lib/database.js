...
let campusMemoryCache = { text: null, expiry: 0 };

export async function getCampusMemoryContext() {
  const now = Date.now();
  if (campusMemoryCache.text !== null && now < campusMemoryCache.expiry) {
    return campusMemoryCache.text;
  }

  const memories = await getCampusMemory();
  if (!memories.length) {
    campusMemoryCache = { text: '', expiry: now + 300000 };
    return '';
  }
  ...
  campusMemoryCache = { text: context, expiry: now + 300000 };
  return context;
}
