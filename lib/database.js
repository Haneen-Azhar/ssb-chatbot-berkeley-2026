...
let campusMemoryCache = { data: null, expiry: 0 };

const MAX_CAMPUS_MEMORY_CHARS = 6000;

export async function getCampusMemoryContext(query = '') {
  const now = Date.now();
  let memories;
  if (campusMemoryCache.data !== null && now < campusMemoryCache.expiry) {
    memories = campusMemoryCache.data;
  } else {
    memories = await getCampusMemory();
    campusMemoryCache = { data: memories, expiry: now + 300000 };
  }
  ...
  if (!query) {
    ...
    if (context.length > MAX_CAMPUS_MEMORY_CHARS) {
      context = context.slice(0, MAX_CAMPUS_MEMORY_CHARS) + '\n[...truncated]';
    }
    return context;
  }
  ...
  for (const m of scored) {
    ...
    const entry = `\n--- ${m.title} ---\n${excerpt}\n`;
    if (totalLen + entry.length > MAX_CAMPUS_MEMORY_CHARS) break;
    context += entry;
