...
export async function getAdminOverview() {
  ...
  try {
    ...
    const [totalRes, todayRes, weekRes, activeRes, avgRes] = await Promise.all([
      ...
      supabase.from('queries').select('*', { count: 'exact', head: true }).gte('created_at', todayStart),
      supabase.from('queries').select('*', { count: 'exact', head: true }).gte('created_at', weekStart),
      supabase.from('queries').select('user_id').gte('created_at', weekStart),
      supabase.from('queries').select('response_time_ms'),
    ]);
...
export async function getAdminUsers() {
  ...
  try {
    const [profilesRes, queriesRes] = await Promise.all([
      ...
      supabase.from('queries').select('user_id, created_at'),
    ]);
